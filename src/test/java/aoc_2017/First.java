package aoc_2017;

import org.junit.Test;

import java.util.function.BiFunction;

import static org.junit.Assert.assertEquals;

public class First {

    public static final String INPUT = "";

    public int solve(int... digits) {
        return solve(this::otherDigitLocationNext, digits);
    }

    public int solve(BiFunction<Integer, Integer, Integer> otherDigitFunction, int... digits) {
        if (digits == null) throw new NullPointerException("Input cannot be null.");
        int listSize = digits.length;
        if (listSize == 0 || listSize == 1) {
            return 0;
        }
        int sum = 0;
        int currentDigitId = 0;
        for (; currentDigitId < listSize; ) {
            int currentDigit = digits[currentDigitId];
            int nextDigit = digits[otherDigitFunction.apply(currentDigitId, listSize)];

            if (currentDigit == nextDigit) {
                sum += currentDigit;
            }
            currentDigitId++;
        }
        return sum;
    }

    private int otherDigitLocationNext(int currentDigitId, int listSize) {
        int nextId = currentDigitId + 1;
        return nextId < listSize ? nextId : 0;
    }

    private int otherDigitLocationHalfway(int currentDigitId, int listSize) {
        int nextId = currentDigitId + (listSize / 2);
        return nextId % listSize;
    }

    @Test
    public void test1() {
        assertEquals(3, solve(1, 1, 2, 2));
    }

    @Test
    public void test2() {
        assertEquals(4, solve(1, 1, 1, 1));
    }

    @Test
    public void test3() {
        assertEquals(0, solve(1, 2, 3, 4));
    }

    @Test
    public void test4() {
        assertEquals(9, solve(9, 1, 2, 1, 2, 1, 2, 9));
    }

    @Test
    public void realCase() {
        int[] digits = convertStringInputToDigitArray();
        System.out.println("Solution is :" + solve(digits));
    }

    private int[] convertStringInputToDigitArray() {
        int[] digits = new int[INPUT.length()];
        char[] chars = INPUT.toCharArray();
        for (int i = 0; i < chars.length; i++) {
            digits[i] = Integer.parseInt("" + chars[i]);
        }
        return digits;
    }

    @Test
    public void testV2_1() {
        assertEquals(6, solve(this::otherDigitLocationHalfway, 1, 2, 1, 2));
    }

    @Test
    public void testV2_2() {
        assertEquals(0, solve(this::otherDigitLocationHalfway, 1, 2, 2, 1));
    }

    @Test
    public void testV2_3() {
        assertEquals(4, solve(this::otherDigitLocationHalfway, 1, 2, 3, 4, 2, 5));
    }

    @Test
    public void testV2_4() {
        assertEquals(12, solve(this::otherDigitLocationHalfway, 1, 2, 3, 1, 2, 3));
    }

    @Test
    public void testV2_5() {
        assertEquals(4, solve(this::otherDigitLocationHalfway, 1, 2, 1, 3, 1, 4, 1, 5));
    }

    @Test
    public void realCaseV2() {
        int[] digits = convertStringInputToDigitArray();
        System.out.println("Solution is :" + solve(this::otherDigitLocationHalfway, digits));
    }
}
